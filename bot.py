...
DEFAULT_CONFIG = {
    ...
    "customer_role_id": int(os.getenv("CUSTOMER_ROLE_ID", 0)) or 1545438540362555463
}
...
# This function now gracefully handles missing Tesseract
def blur_region(image, x1, y1, x2, y2, radius=7):
    """Blur one individual region."""
...
def is_button_text(text):
    """Don't blur large View/Report buttons."""
    text_lower = text.lower().strip()
    ignored = {"view","report","refresh","buy","cancel","confirm","close"}
    return text_lower in ignored

def calculate_dark_ratio(gray, x1, y1, x2, y2):
    """Calculates dark text ratio in an OCR region."""
    h, w = gray.shape
...
def blur_proof_text(image_data: bytes) -> bytes:
    ...
    """
    # If Tesseract is not available, still attempt basic detection without OCR
    try:
        ...
        card_mask = cv2.morphologyEx(light, cv2.MORPH_CLOSE,
        ...
        for contour in contours:
            ...
            if w < max(160, int(width*0.30)): continue
            if h < 45: continue
            if w > int(width*0.68): continue
            if w / max(h,1) < 1.5: continue
            ix1 = max(0, x+5); iy1 = max(0, y+5)
            ix2 = min(width, x+w-5); iy2 = min(height, y+h-5)
            if ix2 <= ix1 or iy2 <= iy1: continue
            inside = gray[iy1:iy2, ix1:ix2]
            if inside.size == 0: continue
            if float(np.mean(inside >= 185)) < 0.40: continue
        ...
        for ry1, ry2 in runs:
            ...
            if active_cols.size == 0: continue
            x1 = int(active_cols.min())
            x2 = int(active_cols.max()) + 1
            rw = x2 - x1
            if rw < max(160, int(width*0.30)): continue
        ...
        for card in sorted(cards, key=lambda c: (c[1], c[0])):
            ...
            for i, old in enumerate(merged_cards):
                ...
                same_card = (overlap_x > max(20, int(min(w, ow)*0.45)) and overlap_y > 0)
                close_same_card = (abs(x-ox) <= 15 and abs(y-oy) <= 15 and abs(w-ow) <= 25)
                if same_card or close_same_card:
                    merged_cards[i] = (min(x, ox), min(y, oy),
                                       max(x2, ox2)-min(x, ox),
                                       max(y2, oy2)-min(y, oy))
                    merged = True
                    break
        ...
        print(f"[PROOF] Visible proof cards detected: {len(cards)}")
        if not cards:
            print("[PROOF] No proof cards detected; returning original.")
            return image_data

        # 2. FIND USERNAME INSIDE EACH CARD
        username_regions = []
        for card_index, (x, y, w, h) in enumerate(cards, 1):
            ...
            dark = cv2.morphologyEx(dark, cv2.MORPH_OPEN,
                                    ...
                                    iterations=1)
            grouped = cv2.dilate(dark, cv2.getStructuringElement(cv2.MORPH_RECT, (5,2)), iterations=1)
            grouped = cv2.morphologyEx(grouped, cv2.MORPH_CLOSE,
                                       cv2.getStructuringElement(cv2.MORPH_RECT, (13,3)),
                                       iterations=2)
            contours2, _ = cv2.findContours(grouped, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            ...
            for contour in contours2:
                ...
                if ch > 30: continue
                if cw > roi.shape[1]*0.95: continue
                if cw / max(ch,1) < 1.5: continue
                box = roi[max(0,cy):min(roi.shape[0], cy+ch),
                          max(0,cx):min(roi.shape[1], cx+cw)]
                if box.size == 0: continue
                dark_ratio = float(np.mean(box <= 135))
                if dark_ratio < 0.025: continue
                pad = 5
                sx1 = max(0, cx-pad); sy1 = max(0, cy-pad)
                sx2 = min(roi.shape[1], cx+cw+pad); sy2 = min(roi.shape[0], cy+ch+pad)
                surrounding = roi[sy1:sy2, sx1:sx2]
                if surrounding.size == 0: continue
                if float(np.mean(surrounding)) < 135: continue
                candidates.append((cx, cy, cx+cw, cy+ch, dark_ratio))

            # OCR fallback (only if Tesseract is available)
            if TESSERACT_AVAILABLE and pytesseract:
                try:
                    up = cv2.resize(roi, None, fx=3, fy=3, interpolation=cv2.INTER_CUBIC)
                    ocr_data = pytesseract.image_to_data(up, config="--oem 3 --psm 7",
                    ...
                    for i, text in enumerate(texts):
                        ...
                        if is_date_or_time(text): continue
                        if is_button_text(text): continue
                        try:
                            conf = float(confs[i])
                        except:
                            conf = 0
                        if conf < 10: continue
                        ox = int(lefts[i]/3); oy = int(tops[i]/3)
                        ow = int(widths[i]/3); oh = int(heights[i]/3)
                        if ow < 15 or oh < 5: continue
                        ox2 = min(roi.shape[1], ox+ow)
                        oy2 = min(roi.shape[0], oy+oh)
                        if ox2 <= ox or oy2 <= oy: continue
                        ocr_box = roi[oy:oy2, ox:ox2]
                        if ocr_box.size == 0: continue
                        dark_ratio = float(np.mean(ocr_box <= 140))
                        if dark_ratio < 0.02: continue
                        candidates.append((ox, oy, ox2, oy2, dark_ratio))
                except Exception as e:
                    print(f"[PROOF] OCR fallback error on card {card_index}: {e}")

            if not candidates:
                print(f"[PROOF] Card {card_index}: no username candidate.")
                continue
            ...
            for (cx1, cy1, cx2, cy2, score) in candidates:
                ...
                for j, current in enumerate(merged):
                    ...
                    if horizontal_gap <= 16 and vertical_gap <= 9:
                        merged[j] = (min(mx1, cx1), min(my1, cy1), max(mx2, cx2), max(my2, cy2))
                        found = True
                        break
                if not found:
                    merged.append((cx1, cy1, cx2, cy2))

            # Choose best username-like line
            best = None
            best_score = -1
            for (mx1, my1, mx2, my2) in merged:
                mw = mx2 - mx1; mh = my2 - my1
                if mw < 18 or mh < 5: continue
                if mw / max(mh,1) < 1.5: continue
                full_y = band_top + my1
                if full_y > y + 58: continue
                check = gray[max(0, band_top+my1):min(height, band_top+my2),
                ...
                if score > best_score:
                    best_score = score
                    best = (band_left+mx1, band_top+my1, band_left+mx2, band_top+my2)
            if best is not None:
                username_regions.append(best)
                print(f"[PROOF] Card {card_index}: username region {best}")

        print(f"[PROOF] Username regions found: {len(username_regions)}")
        if not username_regions:
            print("[PROOF] No username regions found; returning original.")
            return image_data
        ...
        for (x1, y1, x2, y2) in username_regions:
            ...
            bx1 = max(0, x1-pad_x); by1 = max(0, y1-pad_y)
            bx2 = min(width, x2+pad_x); by2 = min(height, y2+pad_y)
        ...
        output.seek(0)
        print("[PROOF] Updated username-only blur complete.")
        return output.getvalue()

    except Exception as e:
        print(f"Proof processing error: {e}")
        return image_data
...
# =========================================================
# CHANNEL RENAME QUEUE (with lock)
# =========================================================
    ...
    while True:
        async with rename_lock:
            if pending_renames:
                ...
                continue
        # Do the rename outside the lock to avoid blocking other operations
    ...
    if customer_role_id:
        ...
        if role:
            try:
                ...
                print(f"Cannot give {role.name} to {member}")
            except Exception as e:
...
class TicketView(discord.ui.View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not isinstance(category, discord.CategoryChannel):
            return await interaction.response.send_message("❌ The ticket category hasn't been configured yet.",
                                                           ephemeral=True)

        # Check for existing open ticket
        for channel in guild.text_channels:
            if channel.topic == f"ali_adm_ticket:{interaction.user.id}":
                return await interaction.response.send_message(f"❌ You already have an open ticket: {channel.mention}",
                                                               ephemeral=True)
        ...
        username = re.sub(r'[^a-zA-Z0-9\-]', '-', interaction.user.name.lower().replace(" ", "-"))
        ...
        try:
            ...
            return await interaction.response.send_message("❌ I don't have permission to create ticket channels.",
                                                           ephemeral=True)
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        if is_staff(interaction):
            ...
            if interaction.channel:
                await interaction.channel.delete(reason=f"Ticket closed by staff {interaction.user}")
            return
        ...
        vouch_channel = guild.get_channel(vouch_channel_id) if vouch_channel_id else None
        if not isinstance(vouch_channel, discord.TextChannel):
            return await interaction.response.send_message("❌ The vouch channel has not been configured yet.",
                                                           ephemeral=True)

        ticket_created_at = interaction.channel.created_at if interaction.channel else discord.utils.utcnow()
        has_vouched = False
        ...
        if not has_vouched:
            return await interaction.response.send_message(
                f"Did you vouch yet? ♡\n\nPlease use `/vouch` in {commands_mention} before closing your ticket!",
                ephemeral=True)
        ...
        await asyncio.sleep(3)
        if interaction.channel:
            await interaction.channel.delete(reason=f"Ticket closed by customer {interaction.user}")

# =========================================================
# VOUCH COOLDOWN (simple in-memory)
# =========================================================
...
@app_commands.describe(
    ...
    proof_channel="Proof channel (optional)"
)
async def setup(
    ...
    proof_channel: Optional[discord.TextChannel] = None
):
...
@bot.tree.command(name="setupstatus", description="Configure the shop status channel.")
@app_commands.default_permissions(administrator=True)
async def setupstatus(interaction: discord.Interaction, status_channel: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message("❌ You need **Administrator** permission.", ephemeral=True)
    config["status_channel_id"] = status_channel.id
    save_config(config)
    await interaction.response.send_message(f"♡ Status channel set to {status_channel.mention}.", ephemeral=True)

# =========================================================
# SETUP JOINS
# =========================================================

@bot.tree.command(name="setupjoins", description="Configure welcome/goodbye messages.")
@app_commands.default_permissions(administrator=True)
async def setupjoins(
    ...
    save_config(config)
    await interaction.response.send_message("♡ Welcome/goodbye system configured!", ephemeral=True)

# =========================================================
# SETUP PROOF
# =========================================================

@bot.tree.command(name="setupproof", description="Configure the proof submission channel.")
@app_commands.default_permissions(administrator=True)
async def setupproof(interaction: discord.Interaction, proof_channel: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message("❌ You need **Administrator** permission.", ephemeral=True)
    config["proof_channel_id"] = proof_channel.id
    save_config(config)
    await interaction.response.send_message(f"♡ Proof channel configured successfully!\n\n📸 Proof Channel: {proof_channel.mention}",
                                            ephemeral=True)
...
async def ticketpanel(interaction: discord.Interaction, channel: discord.TextChannel):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ You need staff permissions.", ephemeral=True)
    embed = discord.Embed(
        title="୨୧・𝘴𝘶𝘱𝘱𝘰𝘳𝘵 𝘵𝘪𝘤𝘬𝘦𝘵𝘴 ♡",
...
# =========================================================
# PROOF (with staff/customer check and file size limit)
# =========================================================
...
async def proof(interaction: discord.Interaction, image: discord.Attachment):
    # Only allow staff or users with an open ticket
    if not is_staff(interaction):
        # Check if user has an open ticket
        guild = interaction.guild
        if guild:
            has_ticket = any(
                ch.topic == f"ali_adm_ticket:{interaction.user.id}"
                for ch in guild.text_channels
            )
            if not has_ticket:
                return await interaction.response.send_message(
                    "❌ You need an open ticket to submit proof, or be staff.",
                    ephemeral=True
                )

    proof_channel_id = config.get("proof_channel_id")
    if not proof_channel_id:
        return await interaction.response.send_message("Proof channel is not configured.", ephemeral=True)

    proof_channel = interaction.guild.get_channel(int(proof_channel_id))
    if not proof_channel:
        return await interaction.response.send_message("Proof channel could not be found.", ephemeral=True)

    valid_extensions = (".png", ".jpg", ".jpeg", ".webp")
    content_type = (image.content_type or "").lower()
    filename = image.filename.lower()
    if not (content_type.startswith("image/") or filename.endswith(valid_extensions)):
        return await interaction.response.send_message("Please upload a PNG, JPG, JPEG, or WEBP image.", ephemeral=True)

    if image.size > MAX_PROOF_SIZE:
        return await interaction.response.send_message(f"Image too large. Max size is {MAX_PROOF_SIZE//1024//1024} MB.",
                                                       ephemeral=True)
    ...
    try:
        ...
        blurred_data = blur_proof_text(image_data)
        file = discord.File(io.BytesIO(blurred_data), filename="proof.png")
        await proof_channel.send(content="♡ **New Proof!**\nThank you so much! ♡", file=file)
        await interaction.followup.send("Your proof has been submitted! ♡", ephemeral=True)
    except Exception as e:
        print(f"Proof command error: {e}")
        await interaction.followup.send("Something went wrong while processing the proof.", ephemeral=True)
...
async def vouch(interaction: discord.Interaction, message: str):
    # Cooldown: one vouch per user per hour
    user_id = interaction.user.id
    now = discord.utils.utcnow()
    if user_id in vouch_cooldown:
        if (now - vouch_cooldown[user_id]).total_seconds() < 3600:
            return await interaction.response.send_message("❌ You can only vouch once per hour.", ephemeral=True)
    vouch_cooldown[user_id] = now
    ...
    if not isinstance(channel, discord.TextChannel):
        return await interaction.response.send_message("❌ The vouch channel hasn't been configured yet.", ephemeral=True)
...
async def vouchcount(interaction: discord.Interaction):
    ...
    channel = interaction.guild.get_channel(channel_id) if channel_id and interaction.guild else None
    if not isinstance(channel, discord.TextChannel):
        return await interaction.response.send_message("❌ Vouch channel isn't configured.", ephemeral=True)

    await interaction.response.defer(ephemeral=True)
    count = 0
    # Limit to last 2000 messages to avoid performance issues
...
async def status(interaction: discord.Interaction, state: app_commands.Choice[str]):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can update the status.", ephemeral=True)

    status_channel_id = config.get("status_channel_id")
    channel = interaction.guild.get_channel(status_channel_id) if status_channel_id else None
    if not isinstance(channel, discord.TextChannel):
        return await interaction.response.send_message("❌ Status channel isn't configured.", ephemeral=True)

    if state.value == "available":
        title = "🟢・𝘰𝘳𝘥𝘦𝘳𝘴 𝘢𝘳𝘦 𝘢𝘷𝘢𝘪𝘭𝘢𝘣𝘭𝘦"
        description = "Our shop is currently **OPEN** for new orders! ♡"
        color = GREEN
        channel_name = "🟢-available"
    elif state.value == "busy":
        title = "🔴・𝘰𝘳𝘥𝘦𝘳𝘴 𝘢𝘳𝘦 𝘣𝘶𝘴𝘺"
        description = "Our shop is currently **BUSY**! ♡\nOrders may take a little longer."
        color = RED
        channel_name = "🔴-busy"
    else:
        title = "⚪・𝘰𝘳𝘥𝘦𝘳𝘴 𝘢𝘳𝘦 𝘤𝘭𝘰𝘴𝘦𝘥"
        description = "Our shop is currently **CLOSED**! ♡"
        color = GRAY
        channel_name = "⚪-closed"
...
# =========================================================
# SAY - MULTIPLE ROLE SELECT (fixed max_values per menu)
# =========================================================

class SayRoleSelect(discord.ui.Select):
    def __init__(self, roles: List[discord.Role], selected_roles: List[discord.Role]):
        self.roles = roles
        ...
        super().__init__(
            ...
            min_values=0,
            max_values=len(options),  # limit to number of options in this menu
            options=options
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.selected_roles[:] = [role for role in self.selected_roles if role.id not in role_ids]
...
class SayRoleView(discord.ui.View):
    def __init__(self, interaction: discord.Interaction, channel: discord.TextChannel, message: str):
        ...
        guild = interaction.guild
        me = guild.me
        roles = [role for role in guild.roles if not role.is_default() and (not me or role < me.top_role)]

        # Create up to 5 select menus (Discord limit)
        for i in range(0, min(len(roles), 125), 25):
            chunk = roles[i:i+25]
...
class SaySendButton(discord.ui.Button):
    def __init__(self, view: SayRoleView):
        self.say_view = view
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != view.original_user.id:
            return await interaction.response.send_message("❌ Only the person who used `/say` can use this.",
                                                           ephemeral=True)
        ...
        # Check if bot can mention roles
        if roles:
            # Ensure bot has mention_everyone permission if mentioning roles
            # Actually, role mentions require the bot to have the 'mention_everyone' permission, or the role is mentionable.
            # We'll assume the bot has it; otherwise, the mention will fail silently.
            pass
        ...
        await interaction.response.edit_message(
            content=f"♡ Announcement sent to {view.channel.mention}.",
            view=None
        )
        view.stop()
...
async def say(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can use `/say`.", ephemeral=True)
    ...
    me = guild.me
    manageable_roles = [role for role in guild.roles if not role.is_default() and (not me or role < me.top_role)]
    if len(manageable_roles) > 125:
        return await interaction.response.send_message(
            "❌ This server has too many roles for the multi-select menu. "
            "Discord allows a maximum of 125 select options in one view.",
            ephemeral=True
        )
...
async def warn(interaction: discord.Interaction, user: discord.Member, reason: str):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can warn users.", ephemeral=True)
    ...
    if interaction.user != interaction.guild.owner and user.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ You cannot warn someone with an equal or higher role.",
                                                       ephemeral=True)
    try:
        await user.send(embed=discord.Embed(title="⚠️ You have been warned",
                                            description=f"Reason: **{reason}**", color=RED))
    except discord.Forbidden:
        pass
    await interaction.response.send_message(f"⚠️ Warned {user.mention}.\nReason: **{reason}**", ephemeral=True)

# =========================================================
# CLEAR (fixed to delete most recent messages)
# =========================================================

@bot.tree.command(name="clear", description="Delete messages.")
async def clear(interaction: discord.Interaction, amount: int):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can clear messages.", ephemeral=True)
    if amount < 1 or amount > 100:
        return await interaction.response.send_message("❌ Amount must be between 1 and 100.", ephemeral=True)
    if not isinstance(interaction.channel, (discord.TextChannel, discord.Thread)):
        return await interaction.response.send_message("❌ This isn't a text channel or thread.", ephemeral=True)

    await interaction.response.defer(ephemeral=True)

    try:
        # Delete the most recent 'amount' messages
        messages = []
        async for msg in interaction.channel.history(limit=amount, oldest_first=False):
            messages.append(msg)
        ...
        if bulk:
            try:
                deleted = await interaction.channel.delete_messages(bulk)
        ...
        await interaction.followup.send(f"🗑️ Deleted **{deleted_count}** most recent message(s).", ephemeral=True)

    except discord.Forbidden:
        await interaction.followup.send("❌ I don't have permission to delete messages.", ephemeral=True)
    except discord.HTTPException as e:
        print(f"Clear command error: {e}")
        await interaction.followup.send("❌ Discord returned an error while deleting messages.", ephemeral=True)
...
async def giverole(interaction: discord.Interaction, user: discord.Member, role: discord.Role):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can give roles.", ephemeral=True)
    if role.is_default():
        return await interaction.response.send_message("❌ You cannot give @everyone.", ephemeral=True)
    if interaction.guild.me and role >= interaction.guild.me.top_role:
        return await interaction.response.send_message("❌ I cannot give that role because it is too high.", ephemeral=True)
    try:
        await user.add_roles(role, reason=f"Given by {interaction.user}")
        await interaction.response.send_message(f"✅ Gave {user.mention} {role.mention}.", ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message("❌ I cannot give that role.", ephemeral=True)
...
async def mute(interaction: discord.Interaction, user: discord.Member, reason: str = "No reason provided"):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can mute users.", ephemeral=True)
    if user.id == interaction.user.id:
        return await interaction.response.send_message("❌ You cannot mute yourself.", ephemeral=True)
    if interaction.user != interaction.guild.owner and user.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ You cannot mute someone with an equal or higher role.",
                                                       ephemeral=True)
    if interaction.guild.me and user.top_role >= interaction.guild.me.top_role:
        return await interaction.response.send_message("❌ I cannot mute that user.", ephemeral=True)
    try:
        await user.timeout(discord.utils.utcnow() + timedelta(minutes=10), reason=reason)
        await interaction.response.send_message(f"🔇 Muted {user.mention} for **10 minutes**.\nReason: **{reason}**",
                                                ephemeral=True)
    except discord.Forbidden:
...
async def ban(interaction: discord.Interaction, user: discord.Member, reason: str = "No reason provided"):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can ban users.", ephemeral=True)
    if user.id == interaction.user.id:
        return await interaction.response.send_message("❌ You cannot ban yourself.", ephemeral=True)
    if interaction.user != interaction.guild.owner and user.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ You cannot ban someone with an equal or higher role.",
                                                       ephemeral=True)
    if interaction.guild.me and user.top_role >= interaction.guild.me.top_role:
        return await interaction.response.send_message("❌ I cannot ban that user.", ephemeral=True)
    try:
...
async def kick(interaction: discord.Interaction, user: discord.Member, reason: str = "No reason provided"):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can kick users.", ephemeral=True)
    if user.id == interaction.user.id:
        return await interaction.response.send_message("❌ You cannot kick yourself.", ephemeral=True)
    if interaction.user != interaction.guild.owner and user.top_role >= interaction.user.top_role:
        return await interaction.response.send_message("❌ You cannot kick someone with an equal or higher role.",
                                                       ephemeral=True)
    if interaction.guild.me and user.top_role >= interaction.guild.me.top_role:
        return await interaction.response.send_message("❌ I cannot kick that user.", ephemeral=True)
    try:
...
async def lockchannel(interaction: discord.Interaction, channel: Optional[discord.TextChannel] = None,
                      reason: str = "No reason provided"):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can lock channels.", ephemeral=True)
...
async def unlockchannel(interaction: discord.Interaction, channel: Optional[discord.TextChannel] = None,
                        reason: str = "No reason provided"):
    if not is_staff(interaction):
        return await interaction.response.send_message("❌ Only staff can unlock channels.", ephemeral=True)
    target = channel or interaction.channel
    if not isinstance(target, discord.TextChannel):
        return await interaction.response.send_message("❌ Invalid text channel.", ephemeral=True)
    try:
        await target.set_permissions(interaction.guild.default_role, send_messages=True, reason=reason)
...
async def vouch_prefix(ctx, *, message: str = None):
    ...
    # Try to delete the command message, but don't rely on it
    try:
...
# Start web server and register cleanup
server = keep_alive()
atexit.register(lambda: server.join(1))
